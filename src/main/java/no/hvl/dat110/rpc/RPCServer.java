		...
		while (!stop) {
		   ...
		   // - send back the message containing RPC reply
			try {
				...
				byte[] params = new byte[requestmsg.getData().length - 1];
				System.arraycopy(requestmsg.getData(), 1, params, 0, params.length);
				byte[] reply = services.get(rpcid).invoke(params);
